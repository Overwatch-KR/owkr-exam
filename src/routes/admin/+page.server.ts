...
function questionInput(data: FormData) {
	...
	if (
		type === 'multiple' &&
		(options.length < 2 || !Number.isInteger(correctAnswer) || correctAnswer < 0 || correctAnswer >= options.length)
	) {
...
export const load = async (event) => {
	...
	if (!section) {
		...
		const selectedQuestionId = section === 'questions' ? event.url.searchParams.get('question') : null;
...
export const actions = {
		...
		updateQuestion: async (event) => {
			...
			const parsed = questionInput(data);
			if ('error' in parsed) return fail(400, {message: parsed.error});
			const [existing] = await database().select({id: questions.id}).from(questions).where(eq(questions.id, id));
