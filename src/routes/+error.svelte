...
<main class="bg-paper min-h-screen">
	...
	<section class="mx-auto flex min-h-[calc(100vh-64px)] max-w-5xl items-center px-5 py-16">
		<div class="w-full max-w-xl">
			<div
				class="bg-sky text-navy mb-7 flex h-12 w-12 items-center justify-center rounded-full"
				aria-hidden="true"
			>
				<svg
					viewBox="0 0 24 24"
			...
			<p class="text-navy font-mono text-xs font-bold tracking-[0.12em]">ERROR {page.status}</p>
			<h1 class="text-ink mt-3 text-[32px] font-bold tracking-[-0.045em] sm:text-[40px]">
				{content.title}
			</h1>
			<p class="text-muted mt-4 max-w-md text-sm leading-6 sm:text-base">{content.description}</p>

			<div class="border-line mt-9 flex flex-wrap gap-3 border-t pt-6">
