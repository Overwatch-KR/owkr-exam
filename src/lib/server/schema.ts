import {pgTable, text, timestamp, integer, jsonb, boolean, uuid, uniqueIndex} from 'drizzle-orm/pg-core';
...
export const attempts = pgTable('exam_attempts', {
		id: uuid('id').defaultRandom().primaryKey(),
		codeId: uuid('code_id').notNull().references(() => examCodes.id),
		discordId: text('discord_id').notNull(),
...
export const answers = pgTable('answers', {
		id: uuid('id').defaultRandom().primaryKey(),
		attemptQuestionId: uuid('attempt_question_id').notNull().references(() => examQuestions.id, {onDelete: 'cascade'}),
